<template>
    <NavbarView />
    <div class="notif-card card shadow-sm p-3 bg-white rounded mx-auto mt-4">
        ...
        <div class="card-body overflow-auto">
            <ul class="list-group">
                <li v-for="notification in notifications" :key="notification.notification_id"
                    class="list-group-item d-flex justify-content-between align-items-start text-left bordered"
                    :class="[{ 'bg-aliceblue': notification.status === 'unread' }]">
                    <div>
                        <p :class="['mb-1', { 'text-muted': notification.status === 'read' }]">
                            {{ notification.message }}
                        </p>
                        <small class="text-muted">{{ formatDate(notification.created_at) }}</small>
                    </div>
                    <button v-if="notification.status === 'unread'" class="btn btn-sm btn-outline-success"
        ...
        <div>
            <router-link to="/home" class="btn btn-sm btn-secondary">
                Back
            </router-link>
        </div>
...
.notif-card {
    width: 90%;
    /* Mengambil 90% dari lebar layar */
    max-width: 600px;
    /* Lebar maksimum tetap 600px */
    height: 80vh;
    /* Tinggi responsif berdasarkan viewport height */
    max-height: 600px;
    /* Tinggi maksimum tetap 600px */
    margin: 0 auto;
    /* Memusatkan card */
}
...
.notif-card .card-body {
    ...
    /* Mengurangi tinggi header card */
    overflow-y: auto;
    /* Scroll hanya untuk konten berlebih */
}
